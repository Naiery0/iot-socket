...
int main(int argc, char* argv[]) {
    ...
    epfd = epoll_create1(0); // 1. epoll 인스턴스 
    ev.events = EPOLLIN;	// epoll 이벤트 구조체
    ev.data.fd = server_fd;	// 관찰할 fd로 server
	// 2. 서버 fd의 읽기 이벤트를 감지하도록 epfd인스턴스
    epoll_ctl(epfd, EPOLL_CTL_ADD, server_fd, &ev);

    while (1) {
		// 3. 이벤트 발생 대기, 인스턴스에 이벤트가 발생하면
